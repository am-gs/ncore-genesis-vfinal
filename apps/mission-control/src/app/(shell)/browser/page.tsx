    ...
    <div className="flex flex-col h-[calc(100vh-5rem)] gap-4">
      ...
      <div className="flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-gradient">Manus Browser</h1>
            <p className="text-xs text-muted">Live browser viewport for agent navigation</p>
          </div>
        </div>
      ...
      <div className="flex-1 min-h-0">
        {loading ? (
          ...
          <BrowserViewport taskId={selectedId} fullScreen />
        ) : (
          <div className="h-full rounded-2xl border border-white/[0.06] bg-[rgba(15,23,42,0.55)] backdrop-blur-xl flex items-center justify-center">
            <div className="text-center">
              <Globe className="h-12 w-12 text-muted/20 mx-auto mb-3" />
              <div className="text-sm text-muted">Select a task to view its browser viewport</div>
            </div>
          </div>
        )}
      </div>
